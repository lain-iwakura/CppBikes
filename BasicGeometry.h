...
//
// Point - класс точки, задается тремя координатами в глобальной СК.
...
// В момент привязки объекта (точки или вектора) к базису в объект помещается указатель
// на этот базис, а сам базис хранит указатели на все привязанные к нему объекты.
// Если привязанный к базису объект удаляется, то и указатель на данный объект
// в соответствующем базисе удаляется. 
// Если базис удаляется, то удаляются указатели на данный базис во всех
// привязанных к нему объектах, другими словами, 
// все объекты, привязанные к данному базису, переводятся в глобальную СК.   
...
const TMETRIC TMETRIC_O=0.0000001; // точность TMETRIC (предполагаемая размерность километры => точность 0.1 мм)
typedef double TAMETRIC; // тип для хранения угловых величин 
const TAMETRIC TAMETRIC_O=0.0000001; // точность TAMETRIC (предполагаемая размерность радианы => точность ~ 5.73e-6 градусов ~ 0.021 секунд )
...
{
	bool _transient; // _transient=true => временный объект.
	// Временный объект не имеет привязки к базису в котором находится,
	// т.е. в *basis не хранится указатель на данный объект.
	
	bool _null;	 // _null=true - нулевая точка (точка не существует)	
	
public:
	Basis *basis; // локальный базис (по умолчанию basis=0 - глобальный базис, т.е. локальные координаты равны глобальным)
	...
	// Консруктор копии:
	// Копируем gx,gy,gz,_null. Привязываемся к базису p.basis. Не копируем: _transient
	Point(const Point &p); 
	...
	bool isNull() const {return _null;} // Нулевая точка?
	bool isTransient()const {return _transient;} // Временный объект?
	
	Point& Set(TMETRIC px, TMETRIC py, TMETRIC pz, Basis *b=0); //Точка по координатам в базисе *b
	...
	Point& SetBasis(Basis *b); // Привязка к базису *b (b=0 означает перейти в глобальный базис, т.е.  [локальный базис] = [глобальный базис])
	Point& SetGlobalBasis(); // Удалить привязку к базису *basis (basis=0)
	Point& ReplaceBasis(Basis *b); // Изменить глобальные координаты точки таким образом, чтобы при переходе в новый базис *b локальные координаты не изменились
	Point& ReplaceGBasis(Basis *b); // Аналогично ReplaceBasis(Basis *b), но считаем, что переходим из глобального базиса (basis -  игнорируется и считается равным 0)
	...
	TMETRIC lx(Basis *b) const; // координата X в локальном базисе *b
	TMETRIC ly(Basis *b) const; // координата Y в локальном базисе *b
	TMETRIC lz(Basis *b) const; // координата Z в локальном базисе *b
	Point inGlobalBasis() const; // та же точка в глобальном базисе
	Point inBasis(Basis *b) const; // та же точка в базисе *b
	Point inReplacedBasis(Basis *b) const; // та же точка в замещенном базисе (аналог ReplaceBasis())
	...
	//////// extra operators ///////
	Point operator [] (Basis &b) const;// возвращает inBasis(&b)
	Point operator [] (Basis *b) const;// возвращает inBasis(b)
	Point& operator >> (Basis &b);// возвращает SetBasis(&b)
	Point& operator >> (Basis *b);// возвращает SetBasis(b);
...
{
private:
	bool _transient; // _transient=true - временный объект (не имеет привязки к базису в котором находится)
	bool _null;  // _null=true - нулевой вектор (несуществующий вектор)
public:
	Basis *basis; // локальный базис (basis=0 - глобальный базис, т.е. локальные проекции равны глобальным)
	...
	// Конструктор копии:
	// Копируем _null,fulcrum,gx,gy,gz, привязываемся к базису v.basis. Не копируем _transient.
	...
	bool isNull()const {return _null;} // нулевой вектор?
	bool isTransient() const{return _transient;} // временный объект?
		
	Vector& Set(TMETRIC vx, TMETRIC vy, TMETRIC vz, Basis *b=0); //Вектор по проекциям в базисе *b
	...
	Vector& SetBasis(Basis *b); // Привязка к базису *b
	Vector& SetGlobalBasis(); // Удалить привязку к базису *basis (basis=0)
	Vector& ReplaceBasis(Basis *b); // Изменить глобальные проекции вектора таким образом, чтобы при переходе в новый базис *b локальные проекции не изменились (а так же вызов fulcrum.ReplaceBasis(b))
	Vector& ReplaceGBasis(Basis *b); // Аналогично ReplaceBasis(Basis *b), но считаем, что переходим из глобального базиса (basis -  игнорируется и считается равным 0)
	...
	TMETRIC lx(Basis *b) const; // проекция на ось OX в локальном базисе *b
	TMETRIC ly(Basis *b) const; // проекция на ось OY в локальном базисе *b
	TMETRIC lz(Basis *b) const; // проекция на ось OZ в локальном базисе *b
	...
	Vector inBasis(Basis *b) const; // тот же вектор в базисе *b
	Vector inGlobalBasis() const; //  тот же вектор в глобальном базисе
	Vector inReplacedBasis(Basis *b) const; // тот же вектор в замещенном базисе *b	

 	TAngle AngleA(const Vector &v) const;  // метод не доработан
...
//	Vector operator +() const;
	...
	TAngle operator ^ (const Vector &v) const;// возвращает AngleA(v)
	Vector operator [] (Basis &b) const;// возвращает inBasis(&b)
	Vector operator [] (Basis *b) const;// возвращает inBasis(b)
	Vector& operator >> (Basis &b);// возвращает SetBasis(&b)
	Vector& operator >> (Basis *b);// возвращает SetBasis(b)
...
public:
	...
	void Add(Point *p); //? Привязать точку 
	void Add(Vector *v); //? Привязать вектор 
	void Remove(Point *p); //? удалить привязку точки 
	void Remove(Vector *v); //? удалить привязку вектора 
	void RemoveAll(); // удалить все привязки
